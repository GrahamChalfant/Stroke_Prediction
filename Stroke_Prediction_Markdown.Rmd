...
tuned <- tune.svm(stroke~., data = oversampled_training, cost=seq(from=0.005, to=1,by=0.005), gamma = 1,   tunecontrol=tune.control(cross=10))
```
...
### Random Forest
```{r}
model_RF <- randomForest(stroke~., oversampled_training)
...
confusionMatrix(prediction_RF, test$stroke, positive='1', mode = "prec_recall")
```
...
decTree <- ctree(stroke ~. , data = oversampled_training)
